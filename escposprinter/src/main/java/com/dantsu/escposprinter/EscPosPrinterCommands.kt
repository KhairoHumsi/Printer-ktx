...
class EscPosPrinterCommands @JvmOverloads constructor(
     ...
     */
    val charsetEncoding: EscPosCharsetEncoding =
        charsetEncoding ?: EscPosCharsetEncoding("Arabic", 22)
     ...
     */
    @Throws(EscPosEncodingException::class)
    fun printQRCode(qrCodeType: Int, text: String, oldSize: Int): EscPosPrinterCommands {
        var size = oldSize
        if (!printerConnection.isConnected()) {
            return this
        }
        ...
        System.arraycopy(
            byteArrayOf(0x1D, 0x28, 0x6B, pL.toByte(), pH.toByte(), 0x31, 0x50, 0x30),
            0,
            qrCodeCommand,
            0,
            8
    ...
    companion object {
        ...
        fun bitmapToBytes(bitmap: Bitmap): ByteArray {
            ...
            val bytesByLine = ceil((bitmapWidth.toFloat() / 8f).toDouble())
                .toInt()
        ...
        fun QRCodeDataToBytes(data: String?, size: Int): ByteArray {
            ...
            if (byteMatrix == null) return initImageCommand(0, 0)
            ...
            var i = 8
            if (coefficient < 1) return initImageCommand(0, 0)
